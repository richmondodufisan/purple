#include "HeatConductionSteadyRealAnisotropic.h"
#include "Material.h"
#include "MooseMesh.h"
...
registerMooseObject("purpleApp", HeatConductionSteadyRealAnisotropic);

InputParameters
HeatConductionSteadyRealAnisotropic::validParams()
{
  InputParameters params = ADKernel::validParams();
  params.addClassDescription("Solve the Heat Equation in the Fourier Space to get the steady state response");
										
  params.addRequiredCoupledVar("imaginary_temp", "the imaginiary part of the temperature variable");
...
HeatConductionSteadyRealAnisotropic::HeatConductionSteadyRealAnisotropic(const InputParameters & parameters)
  : DerivativeMaterialInterface<ADKernel>(parameters),
	...
	_Y(adCoupledValue("imaginary_temp")),
...
ADReal
HeatConductionSteadyRealAnisotropic::computeQpResidual()
{
