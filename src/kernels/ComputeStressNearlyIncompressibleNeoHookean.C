...
{	
	...
	RankTwoTensor S = (mu * J_min_23 * (I  -   ((1.0/3.0) * I_1) * C_inv)) +  (kappa * (std::log(J)) * C_inv);
        ...
        for (int j = 0; j < 3; ++j) 
		{
			for (int k = 0; k < 3; ++k) 
			{
				for (int l = 0; l < 3; ++l) 
				{
					...
					Real term5 = -kappa * (std::log(J)) * C_inv(i, k) * C_inv(l, j);
					
					Real term6 = (kappa/2.0) * C_inv(i, j) * C_inv(k, l);
