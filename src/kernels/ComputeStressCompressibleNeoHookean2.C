#include "ComputeStressCompressibleNeoHookean2.h"
#include <Eigen/Dense>
#include <cmath>

registerMooseObject("purpleApp", ComputeStressCompressibleNeoHookean2);

InputParameters
ComputeStressCompressibleNeoHookean2::validParams()
{
...
ComputeStressCompressibleNeoHookean2::ComputeStressCompressibleNeoHookean2(const InputParameters & parameters)
...
void
ComputeStressCompressibleNeoHookean2::computeQpProperties()
{	
...
RankTwoTensor ComputeStressCompressibleNeoHookean2::computePiolaKStress2(const Real &mu, const Real &lambda, const RankTwoTensor &C)
{	
	// Jacobian
...
RankFourTensor ComputeStressCompressibleNeoHookean2::compute_dSdE(const Real &mu, const Real &lambda, const RankTwoTensor &C)
{	
	// Jacobian
	Real J = std::pow(C.det(), 1.0 / 2.0);
...
RankFourTensor ComputeStressCompressibleNeoHookean2::innerProduct4thOrder(const RankFourTensor &A, const RankFourTensor &B)
{
...
RankFourTensor ComputeStressCompressibleNeoHookean2::kroneckerProduct4thOrder(const RankTwoTensor &A, const RankTwoTensor &B)
{
    // Initialize the rank-four tensor for the result
    RankFourTensor C;
