...
ParsedGenerateNodeset::validParams()
{
  ...
  params.addClassDescription("A MeshGenerator that adds nodes to a nodeset if the "
                             "node satisfies the `combinatorial_geometry` expression.");
  params.addParamNamesToGroup("combinatorial_geometry constant_names constant_expressions", "Parsed expression");
  return params;
}
...
{
  _boundary_names.push_back(getParam<BoundaryName>("new_nodeset_name"));
...
ParsedGenerateNodeset::generate()
{
  ...
  for (const auto node_id : make_range(mesh->n_nodes()))
  {
    ...
    if (evaluate(_func_F))
    {
      ...
      boundary_info.add_node(curr_node, boundary_ids[0]);
    }
  }
