...
[AuxKernels]
  [average_surface_temperature_real]
	...
	use_xyzt = true
	expression = '((temp_trans_real)/(pi*((2 * Rprobe)^2)))*exp((-((x-x0)^2+(y-y0)^2))/((2 * Rprobe)^2))'
	block = transducer_material
  []
  [average_surface_temperature_imag]
	...
	constant_expressions = '${x0_val} ${y0_val} ${probe_radius} 3.14159265359'
	use_xyzt = true
	expression = '((temp_trans_imag)/(pi*((2 * Rprobe)^2)))*exp((-((x-x0)^2+(y-y0)^2))/((2 * Rprobe)^2))'
	block = transducer_material
  []
[]
...
[Functions]
  [heat_source_function]
    ...
    expression = '-((Q0*absorbance)/(pi*((2 * Rpump)^2)))*exp((-((x-x0)^2+(y-y0)^2))/((2 * Rpump)^2))'
