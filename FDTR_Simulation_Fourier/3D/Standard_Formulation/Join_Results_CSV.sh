...
# Output file
output_file="../Fourier_Standard_Med_Coarse_CALIBRATE.csv"
...
for x0 in "${x0_vals_num[@]}"; do
	for freq in "${freq_vals_num[@]}"; do
		...
		while [ $current_iteration -le $n_iterations ]; do
			input_file="FDTR_CALIBRATION_Fourier_Standard_theta_${theta_angle}_freq_${freq}_x0_${x0}_v${current_iteration}_out.csv"
