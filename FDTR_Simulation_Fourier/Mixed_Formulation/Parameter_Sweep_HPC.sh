...
for x0_val_num in "${x0_vals_num[@]}"; do
	for theta_val_num in "${theta_vals_num[@]}"; do
		...
		for freq_val_num in "${freq_vals_num[@]}"; do
			...
			new_filename="${og_filename}_Fourier_Mixed_theta_${theta_val_num}_freq_${freq_val_num}_x0_${x0_val_num}_v1.i"
...
while [ $submission_count -lt $n_iterations ]; do
    if check_squeue; then
		...
		rm *_Fourier_Mixed*${former_sim_ver}*i
		...
		for x0_val_num in "${x0_vals_num[@]}"; do
			for theta_val_num in "${theta_vals_num[@]}"; do
				for freq_val_num in "${freq_vals_num[@]}"; do
					...
					# Create a new filename by appending x0_val to the original filename
					new_filename="${init_filename}_Fourier_Mixed_theta_${theta_val_num}_freq_${freq_val_num}_x0_${x0_val_num}_${new_sim_ver}.i"
					...
					former_sim_output="${init_filename}_Fourier_Mixed_theta_${theta_val_num}_freq_${freq_val_num}_x0_${x0_val_num}_${former_sim_ver}_out.e"
